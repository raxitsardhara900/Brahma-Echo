...
func TestResolveAndValidatePublicIPs(t *testing.T) {
	stubResolveHostIPs(t, func(ctx context.Context, network, host string) ([]net.IP, error) {
		...
		case "mixed-private-first.example":
			return []net.IP{net.ParseIP("10.0.0.8"), net.ParseIP("93.184.216.34")}, nil
		case "dupe.example":
	...
	if _, err := ResolveAndValidatePublicIPs(context.Background(), "mixed-private-first.example"); !errors.Is(err, ErrPrivateInternalIP) {
		t.Fatalf("ResolveAndValidatePublicIPs(mixed-private-first.example) error = %v, want private/internal sentinel", err)
	}
